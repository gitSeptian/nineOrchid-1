...
; (function ($, window, document, undefined) {
    ...
    $.extend(SmartCart.prototype, {
        ...
        _setToolbar: function () {
            ...
            var toolbar = $('<div></div>').addClass('panel-footer sc-toolbar ');
            var toolbarButtonPanel = $('<div class="sc-cart-toolbar form-group">');
            ...
            if (this.options.toolbarSettings.showCartSummary) {
                var panelSubTotal = $('<div class="sc-cart-summary-subtotal row">');
                
                panelSubTotal.append('<div class="input-group pull-right col-sm-12 row"> <div class="form-group">  <label for="payment" class="col-sm-4 control-label">Cara Bayar</label> <div class="col-sm-8"><select name="payment" id="payment" class="form-control"></select></div></div> </div>');
                panelSubTotal.append('<div class="input-group pull-right col-sm-12 row"> <div class="form-group">  <label for="remark" class="col-sm-4 control-label">Remark</label> <div class="col-sm-8"><input type="text" class="form-control" id="remark"></div></div> </div>');
                panelSubTotal.append('<div class="input-group pull-right col-sm-12 row"> <div class="form-group">  <label for="remark" class="col-sm-4 control-label">Total</label> <div class="col-sm-8"><input type="text" class="form-control sc-cart-subtotal" readonly></div></div> </div>');
                panelSubTotal.append('<div class="input-group pull-right col-sm-12 row"> <div class="form-group">  <label for="remark" class="col-sm-4 control-label">Saldo Saat Ini</label> <div class="col-sm-8"><input type="text" class="form-control sc-cart-curr-saldo" readonly></div></div> </div>');
                panelSubTotal.append('<div class="input-group pull-right col-sm-12 row"> <div class="form-group">  <label for="remark" class="col-sm-4 control-label">Saldo Berikutnya</label> <div class="col-sm-8"><input type="text" class="form-control sc-cart-next-saldo" readonly></div></div> </div>');
                //panelSubTotal.append(this.options.lang.subtotal).append(' <span class="sc-cart-subtotal"> 0 </span> <br/>');
                //panelSubTotal.append(' Saldo Saat Ini: <span class="sc-cart-curr-saldo"> 0 </span> <br/>');
                //panelSubTotal.append(' Saldo Berikutnya: <span class="sc-cart-next-saldo"> 0 </span>');
                toolbarSummaryPanel.append(panelSubTotal);
            }


            //panelSubTotal.append('<div class="input-group"><span class="input-group-addon">Sisa Saldo Saat ini</span><input type="text" class="form-control sc-cart-subtotal"></div>');
            //panelSubTotal.append('<div class="input-group"><span class="input-group-addon">Sisa Saldo Berikutnya</span><input type="text" class="form-control"></div>');
        ...
        _addToCart: function (p) {
            ...
            if (!p.hasOwnProperty(this.options.paramSettings.productPrice) || parseFloat(p.product_price) <= 0) {
                $(".sc-cart-count").notify("Price is not set for the item", "error", { style: 'bootstrap' });
        ...
        _addUpdateCartItem: function (p) {
            ...
            if (elmMain && elmMain.length > 0) {
                ...
                var itemSummary = '<div class="sc-cart-item-summary"><label class="sc-cart-item-price">' + this._getMoneyFormatted(p[this.options.paramSettings.productPrice]) + '</label>';
                ...
                itemSummary += ' = <label class="sc-cart-item-amount">' + this._getMoneyFormatted(productAmount) + '</label></div>';
        ...
        _hasCartChange: function () {
            ...
            $('.sc-cart-subtotal', this.element).val(this._getMoneyFormatted(this._getCartSubtotal()));

            var saldo = this._getMoneyFormatted($("#SaldoGempita").val());

            var sisa = parseFloat($("#SaldoGempita").val()) - this._getCartSubtotal();

            $('.sc-cart-curr-saldo', this.element).val(saldo);
            $('.sc-cart-next-saldo', this.element).val(this._getMoneyFormatted(sisa));
        ...
        _submitCart: function () {
            ...
            if (sisa < 0 && $("#payment").val() == "CUT BALANCE") {
                $(".sc-cart-next-saldo").notify("Total pemesanan tidak dapat melebihi saldo", "error", { style: 'bootstrap',position: 'right' });
                //$.notify("Total pemesanan tidak dapat melebihi saldo","error");
                return false;
            }
            //var cart_list = JSON.parse($("#cart_list").val());

            var cart_list = {
                "period_id": $("#periode_id").val(),
                "NIK": $("#NIK").val(),
                "id_prs": $("#id_prs").val(),
                "workplace": $("#workplace").text(),
                "gempita_behalf": $('input[name=optradio]:checked').val(),
                "eligibility": $("#SaldoGempita").val(),
                "gempita_total": 0,
                "gempita_type_payment": $("#payment").val(),
                "warehouse_id": $("#warehouse_id").val(),
                "remark": $("#remark").val(),
                "products": JSON.parse($("#cart_list").val())
            }

            //cart_list.push(data1);
            //cart_list.push(data2);

            //cart_list = formElm.serialize();
                ...
                case 'ajax':
                    ...
                    var ajaxSettings = $.extend(true, {}, {
                        ...
                        beforeSend: function () {
                            ...
                            $("#smartcart").LoadingOverlay("show");
                        },
                        error: function (jqXHR, status, message) {
                            alert(message);
                            mi.cart_element.removeClass('loading');
                            $("#smartcart").LoadingOverlay("hide");
        ...
        _getMoneyFormatted: function (n) {
            n = n - 0;
            return n;//$.number(n,2); //Number(n.toFixed(2)).toLocaleString(this.options.currencySettings.locales, this.options.currencySettings.currencyOptions);
        },
        /* 
    ...
    $.fn.smartCart = function (options) {
        ...
        if (options === undefined || typeof options === 'object') {
            ...
            return this.each(function (key, value) {
                ...
                //if (!$.data(this, "smartCart") || $.data(this, "smartCart") == null) {
